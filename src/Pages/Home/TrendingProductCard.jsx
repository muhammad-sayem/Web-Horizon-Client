...
const TrendingProductCard = ({ treProduct, refetch }) => {
    ...
    return (
        <div>
            <div className="p-5 bg-white shadow-[4px_4px_10px_rgba(0,0,0,0.35)] darkDamageshadow-[1px_1px_8px_#f97d5e] h-full rounded-2xl">
                ...
                <div className="flex gap-x-1">
                    ...
                    <button onClick={() => updateUpvote(_id)}
                        ...
                        className={`group text-gray-600 px-3 py-1 rounded-md flex items-center gap-x-1 ${owner?.email === user?.email || hasUpvoted || role === 'Admin' || role === "Moderator"
                            ...
                            : 'bg-[#f97d5e] text-[#1A2634] hover:cursor-pointer transform transition-transform duration-200 ease-in-out hover:scale-105'}`}
                    >
