...
const ProductDetails = () => {
    ...
    return (
        <div className='w-2/3 mx-auto mt-28 mb-12'>
            <div className='gap-x-8 p-12 shadow-[4px_4px_10px_rgba(0,0,0,0.35)] darkDamageshadow-[1px_1px_8px_#87CEEB] rounded-2xl'>
                <div className='flex flex-col items-center'>
                    <img src={productImage} alt='' className='h-16 w-16' />
                    <h3 className="text-2xl font-black darkDamagetext-[#87CEEB]"> {productName} </h3>
                </div>
                ...
                <div className="my-8">
                    ...
                    <div className="flex gap-x-4 items-center">
                        <p className="text-xl font-black darkDamagetext-[#87CEEB]"> Tags: </p>
                        {tags.map((tag, index) => (
                            <p key={index} className="text-[#1A2634] font-bold darkDamagetext-[#87CEEB] text-md">{tag} |</p>
                        ))}
                    </div>

                    <div className="flex gap-x-3 my-8">
                        <Link to={externalLink} className="flex justify-center items-center gap-x-2 text-xl font-bold border-2 border-black w-1/3 py-2 rounded-full hover:bg-[#1A2634] hover:text-[#87CEEB] darkDamagebg-[#87CEEB] darkDamagetext-[#1A2634] darkDamagehover:border-[#87CEEB]" target="_blank">
                ...
                <div>
                    <h2 className="text-3xl text-[#1A2634] font-bold my-4 darkDamagetext-[#87CEEB]"> Reviews </h2>
                    <Reviews
                        key={product._id}
                        product={product}
                    ></Reviews>
                ...
                <div>
                    <button onClick={() => document.getElementById(`my_modal_${id}`).showModal()} disabled={owner?.email === user?.email || role === "Admin" || role === "Moderator"} className={`px-10 py-3 text-lg font-bold bg-[#1A2634] text-[#87CEEB] darkDamagetext-[#1A2634] darkDamagebg-[#87CEEB] darkDamagehover:bg-[#1A2634] darkDamagehover:text-[#87CEEB] darkDamagehover:border darkDamagehover:border-[#87CEEB] ${owner?.email === user?.email || role === "Admin" || role === "Moderator" ? "bg-gray-500 cursor-not-allowed" : "bg-[#1A2634] text-white"}`}> Add a Review </button>
                </div>
